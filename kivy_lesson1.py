...
class FirstScr(Screen):
   def __init__(self, **kwargs):
       ...
       btn = Button(text= 'Выбор: 1', 
                        size_hint=(0.5, .5), pos_hint={'left': 1})       
       btn_back = ScrButton(self, direction='up', goal='main', text="Назад", 
                        size_hint=(.5, .5), pos_hint={'right': 1})
       vl.add_widget(btn)
       vl.add_widget(btn_back)
       self.add_widget(vl)
       btn.on_press = self.qq
...
class FourthScr(Screen):
   def __init__(self, **kwargs):
       ...
       btn_back = ScrButton(self, direction='left', goal='main', text="Назад", size_hint=(1, .2), pos_hint={'center-x': 0.1})
